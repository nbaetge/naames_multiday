...
ba.plot <-  data %>% 
  ...
  geom_line(aes(color = days), size = 0.7) +
  geom_point(aes(fill = days), shape = 21, size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = "Depth, m", y = expression(paste("Bacterioplankton, cells L"^"-1")), colour = "",  fill = "Day") +
  scale_x_reverse() +
  coord_flip() +
  scale_fill_viridis_d() +
  scale_color_viridis_d() +
...
patchwork <-  (ts.plot + guides(color = F)) + mld.plot  + n.plot + guide_area() + (chl.plot  + guides(fill = F)) + (phyto.plot + guides(fill = F)) + (npp.plot  + guides(color = F)) + (aou.plot  + guides(color = F))  + (ba.plot  + guides(fill = F)) + (leu.plot  + guides(fill = F)) + (doc.plot + guides(fill = F)) 
