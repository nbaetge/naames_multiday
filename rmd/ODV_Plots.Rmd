...
beamt.plot <-  mba %>% 
  ...
  geom_point(data = subset, aes(x = lat, y = z), colour = "White", size = 0.05, shape = 3, alpha = 0.1) +
  geom_point(data = s4_ctd_mark, aes(x = lat, y = z), colour = "Black", size = 0.1, shape = 3, alpha = 0.15) +
  geom_point(data = s6_ctd_mark, aes(x = lat, y = z), colour = "Black", size = 0.1, shape = 3, alpha = 0.15) +
...
phyto.plot <-  mba %>% 
  ggplot(aes(x = lat, y = z)) + 
  geom_raster(aes(fill = zscore)) +
  geom_point(data = subset, aes(x = lat, y = z), colour = "White", size = 1, shape = 3, alpha = 0.6) +
  geom_point(data = s4_mark, aes(x = lat, y = z), colour = "Black", size = 1, shape = 3, alpha = 0.6) +
  geom_point(data = s6_mark, aes(x = lat, y = z), colour = "Black", size = 1, shape = 3, alpha = 0.6) +
  scale_fill_gradientn(colors = rev(odv.colors)) +
  scale_y_reverse(breaks = pretty_breaks()) +
  scale_x_continuous(breaks = pretty_breaks()) +
  labs(y = expression(italic("Depth, m")), x = expression(italic("")), fill = expression(italic(paste("")))) +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 32, fig.width = 16, fig.align = "center", warning = FALSE}

patchwork <- beamt.plot / fl.plot / phyto.plot / ba.plot / bcd.plot 
