...
nmds.plot <- plot_ordination(s4n3, nmds,  title = "NAAMES 2 Station 4 & NAAMES 3") +
  geom_point(aes(fill = as_factor(Station), shape = Cruise), alpha = 0.7, size = 4) +
  stat_ellipse(aes(group = interaction(Cruise, dh)), color = "firebrick", level = 0.95, type = "t", size = 0.7) +
  scale_shape_manual(values = c(4, 21)) +
  scale_fill_viridis_d(end = 0.7) +
  theme_linedraw(base_size = 16) +
  guides( shape = guide_legend(title = "Cruise"), color = F, fill = guide_legend(title = "Station", override.aes = list(shape = 21))) 
#removing one of the plotting layers (there are points within points)
nmds.plot$layers <- nmds.plot$layers[-1]
```
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 8, fig.width = 10, fig.align = "center"}

nmds.plot2 <- plot_ordination(s4n3, nmds,  title = "NAAMES 2 Station 4 & NAAMES 3") +
  geom_point(aes(fill = as_factor(dh), shape = Cruise), alpha = 0.7, size = 4) +
  stat_ellipse(aes(group = interaction(Cruise, dh)), color = "firebrick", level = 0.95, type = "t", size = 0.7) +
  scale_shape_manual(values = c(4, 21)) +
  scale_fill_viridis_d(end = 0.7) +
  theme_linedraw(base_size = 16) +
  guides( shape = guide_legend(title = "Cruise"), color = F, fill = guide_legend(title = "Depth Horizon", override.aes = list(shape = 21))) 
#removing one of the plotting layers (there are points within points)
nmds.plot2$layers <- nmds.plot2$layers[-1]
```
...
alphadiv.plot <- alphadiv %>% 
  filter(Cruise == "NAAMES 2" | z %in% c(5, 25, 100, 150, 200)) %>% 
