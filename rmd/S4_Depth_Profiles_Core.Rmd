...
t.plot <-  ctd %>% 
  ...
  labs(x = "", y = expression(paste("Temperature, ˚C")), colour = "", fill = "Days") +
  scale_x_reverse() +
  coord_flip() +
...
```{r s plot, echo = FALSE, warning = FALSE, message = FALSE}
s.plot <-  ctd %>% 
  filter(Cruise == "AT34", Date != "2016-05-28") %>%
  filter(z <= 300) %>% 
  group_by(Date, z) %>% 
  mutate(ave_s = mean(ave_sal_psu, na.rm = T),
         sd_s = sd(ave_sal_psu, na.rm = T)) %>% 
  ungroup() %>% 
  ggplot(aes(x = z, y = ave_s, group = interaction(Date, CampCN))) +
  geom_ribbon(aes(ymin = ave_s - sd_s, ymax = ave_s + sd_s, fill = days), alpha = 0.1) +
  geom_line(aes(color = days), size = 0.7) +
  labs(x = "", y = expression(paste("Salinity, psu")), colour = "", fill = "Days") +
  scale_x_reverse() +
  coord_flip() +
  # scale_color_date(low = "#0db5e6", high = "#d31f2a") +
  # scale_fill_date(low = "#0db5e6", high = "#d31f2a") +
...
tdaa.plot <-  data %>% 
  ...
  geom_line(aes(color = days), size = 0.7) +
  geom_point(aes(fill = days), shape = 21, size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = "", y = expression(paste("TDAA, mmol C m"^-3)), colour = "", fill = "Days") +
  scale_x_reverse() +
  coord_flip() +
  # scale_fill_date(low = "#0db5e6", high = "#d31f2a") +
  # scale_color_date(low = "#0db5e6", high = "#d31f2a") +
...
bp.plot <-  data %>% 
  ...
  mutate(bp = bp/10^3,
         sd_bp = sd_bp/10^3) %>% 
  ggplot(aes(x = z, y = bp, group = interaction(Date, CampCN))) +
  geom_errorbar(aes(x = z, ymin = bp - sd_bp, ymax = bp + sd_bp, color = days)) +
  geom_line(aes(color = days), size = 0.7) +
  geom_point(aes(fill = days), shape = 21, size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = "", y = expression(paste("BP, mmol C m"^"-3","d"^-1)), colour = "", fill = "Days") +
  scale_x_reverse() +
  coord_flip() +
  # scale_fill_date(low = "#0db5e6", high = "#d31f2a") +
  # scale_color_date(low = "#0db5e6", high = "#d31f2a") +
...
patchwork <- mld.plot  + t.plot +  chl.plot + phyc.plot + phyto.plot +   npp.plot + n.plot + aou.plot + doc.plot  + tdaa.plot + tdaaY.plot + ba.plot + bp.plot + bcd.plot
