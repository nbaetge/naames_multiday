...
nmds_s4s6_core.plot <- plot_ordination(s4s6_core, nmds_s4s6_core,  title = "NAAMES 2 Station 4 & NAAMES 3 Station 6") +
  ...
  stat_ellipse(aes(group = interaction(Cruise2, z_interv)), type = "t") +
...
nmds_n2s4_core.plot <- plot_ordination(n2s4_core, nmds_n2s4_core,  title = "NAAMES 2 Station 4") +
  ...
  geom_point(aes(fill = as.factor(days), shape = z_interv), alpha = 0.7, stroke = 2, size = 4) +
...
nmds_n3s6.plot <- plot_ordination(n3s6, nmds_n3s6,  title = "NAAMES 3 Station 6") +
  stat_ellipse(aes(group = interaction(z_interv)), type = "t") +
  geom_point(aes(fill = as.factor(days), shape = z_interv), alpha = 0.7, stroke = 2, size = 4) +
  scale_shape_manual(values = c(21, 22, 23)) +
  scale_fill_viridis_d() +
  theme_classic2(base_size = 16)
...
subset_sample_IDs_n2s4 <-  row.names(sample.tab2)[sample.tab2$Station == 4 & sample.tab2$Cruise == "AT34" & sample.tab2$eddy == "Core"]
...
```{r}
anova(betadisper(dist_n2s4, sample_info_tab_n2s4$z_interv)) 
```
...
```{r}
adonis(dist_n2s4~sample_info_tab_n2s4$z_interv)
```
