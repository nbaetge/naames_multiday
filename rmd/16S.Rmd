...
rela.counts <-  read_rds("~/GITHUB/naames_multiday/Output/processed_data.rds") %>% 
  ...
  distinct() %>% 
  group_by(days, z_interv) %>% 
  mutate(mean_ba = mean(ba),
         sd_ba = sd(ba)) %>% 
  ungroup() %>% 
  select(days, z_interv, mean_ba, sd_ba) %>% 
  distinct() %>% 
  arrange(days) %>% 
  mutate_at(vars(days), as.character) %>% 
  left_join(., relaheat.data_combine_s4) %>% 
  mutate(relacount = mean_relabund * mean_ba)
...
relaheat_combine_s4 <- relaheat.data_combine_s4 %>%
  ...
  geom_text(aes(label = round(mean_relabund, 3), color = font_col), size = 4) +
...
relacounts_combine_s4 <- rela.counts %>%
  filter(mean_relabund > 0.001) %>%
  select(days, z_interv, cof, mean_relabund, relacount) %>% 
  distinct() %>% 
  mutate(font_col = ifelse(relacount < 1 * 10^10, "white", "black")) %>% 
  ggplot(aes(x = days, y = reorder(cof, relacount))) +
  geom_tile(aes(fill = relacount), color = "white") +
  # scale_color_manual(values = custom.colors) %>%
  # scale_fill_gradientn(colors = rev(matlab.colors)) +
  scale_fill_viridis_b(option = "D",  trans = 'log10') +
  geom_text(aes(label = formatC(relacount, format = "e", digits = 1), color = font_col), size = 4) +
  scale_color_manual(values = c("white" = "white", "black" = "black")) +
  labs(x = "Days", y = "Class: Order: Family", fill = expression(paste("Cells L"^-1))) +
  facet_grid(~factor(z_interv, levels = levels)) +
  theme_classic2(base_size = 16) +
  theme(axis.text.y = element_text(size = 12), legend.position = "top") +
   guides(fill = guide_colourbar(barheight = 2, barwidth = 20, frame.colour = "black", frame.linewidth = 2,ticks.colour = "black", ticks.linewidth = 1), color = F) +
...
relacount_del_combine_s4 <- del_data_combine_s4  %>%
  select(days, z_interv, cof, mean_relabund, del_relabund, del_count) %>% 
  distinct() %>% 
  filter(mean_relabund > 0.001, days > 0) %>%
  mutate(font_col = ifelse(del_count < 0.0, "white", "black")) %>% 
  ggplot(aes(x = days, y = reorder(cof, mean_relabund))) +
  ...
  geom_text(aes(label = formatC(del_count, format = "e", digits = 1), color = font_col), size = 4) +
  scale_color_manual(values = c("white" = "white", "black" = "black")) +
...
relaheat.data_combine_s4_order <- counts_custom.tab_combine_s4_order %>% 
  ...
  mutate(co = paste(c, o, sep = ": ")) %>% 
  group_by(days, z_interv, co) %>% 
  mutate(mean_relabund = mean(relabund, na.rm = T)) %>% 
  ungroup() %>% 
  select(-relabund) %>% 
  distinct() %>% 
  mutate_at(vars(p,c,o, co), str_replace_all, "NA", "Unassigned") %>% 
  arrange(desc(co), mean_relabund)
...
relaheat_combine_s4_order <- rela.counts_order %>% 
  filter(mean_relabund > 0.001) %>%
  select(days, z_interv, co, mean_relabund, relacount) %>% 
  distinct() %>% 
  mutate(font_col = ifelse(mean_relabund < 0.03, "white", "black")) %>% 
  ...
  geom_text(aes(label = round(mean_relabund, 3), color = font_col), size = 4) +
  scale_color_manual(values = c("white" = "white", "black" = "black")) +
  labs(x = "Days", y = "Class: Order", fill = "Relative Abundance") +
  facet_grid(~factor(z_interv, levels = levels)) +
  theme_classic2(base_size = 16) +
  theme(axis.text.y = element_text(size = 12), legend.position = "top") +
   guides(fill = guide_colourbar(barheight = 2, barwidth = 20, frame.colour = "black", frame.linewidth = 2,ticks.colour = "black", ticks.linewidth = 1), color = F) +
...
relacounts_combine_s4_order <- rela.counts_order %>% 
  ...
  distinct() %>% 
  mutate(font_col = ifelse(relacount < 1 * 10^10, "white", "black")) %>% 
  ggplot(aes(x = days, y = reorder(co, relacount))) +
  geom_tile(aes(fill = relacount), color = "white") +
  # scale_color_manual(values = custom.colors) %>%
  # scale_fill_gradientn(colors = rev(matlab.colors)) +
  scale_fill_viridis_b(option = "D",  trans = 'log10') +
...
relacount_del_combine_s4_order <- del_data_combine_s4_order  %>%
  select(days, z_interv, co, mean_relabund, del_relabund, del_count) %>% 
  distinct() %>% 
  filter(mean_relabund > 0.001, days > 0) %>%
  mutate(font_col = ifelse(del_count < 0.0, "white", "black")) %>% 
  ggplot(aes(x = days, y = reorder(co, mean_relabund))) +
...
p2b <- nmds_n2s4_core.plot + 
  guides(fill = guide_colorbar(title = "Days", barheight = 8, barwidth = 2, frame.colour = "black", frame.linewidth = 2,ticks.colour = "black", ticks.linewidth = 1), shape = guide_legend(title = "Depth Horizon"), linetype = guide_legend(title = "Days"), color = F) + theme_classic2(16)
p2c <- nmds_n3s6.plot + 
  guides(fill = guide_colorbar(title = "Days", barheight = 8, barwidth = 2, frame.colour = "black", frame.linewidth = 2,ticks.colour = "black", ticks.linewidth = 1), shape = guide_legend(title = "Depth Horizon"),  linetype = guide_legend(title = "Depth Horizon"), color = F) + theme_classic2(16)
...
p3a <- relaheat_combine_s4 + theme_classic2(16) + theme( legend.position = "top") +
  # theme( legend.position = "top") +
   guides(fill = guide_colourbar(barheight = 2, barwidth = 20, frame.colour = "black", frame.linewidth = 2,ticks.colour = "black", ticks.linewidth = 1), color = F) 
p3b <- relaheat_del_combine_s4 +  theme_classic2(16) + theme( legend.position = "top", axis.text.y = element_blank()) +
  # theme( legend.position = "top") +
   guides(fill = guide_colourbar(barheight = 2, barwidth = 20, frame.colour = "black", frame.linewidth = 2,ticks.colour = "black", ticks.linewidth = 1), color = F) 
...
p4a <- relacounts_combine_s4 + theme_classic2(16) + theme( legend.position = "top") +
  # theme( legend.position = "top") +
   guides(fill = guide_colourbar(barheight = 2, barwidth = 20, frame.colour = "black", frame.linewidth = 2,ticks.colour = "black", ticks.linewidth = 1), color = F) 
p4b <- relacount_del_combine_s4 + theme_classic2(16) + theme( legend.position = "top", axis.text.y = element_blank()) +
  # theme( legend.position = "top") +
   guides(fill = guide_colourbar(barheight = 2, barwidth = 20, frame.colour = "black", frame.linewidth = 2,ticks.colour = "black", ticks.linewidth = 1), color = F) 
...
officer::read_pptx() %>%
  ...
  # export slide 
  base::print(
    target = "~/Desktop/Dissertation/MS_N2S4/Presentations/16s.pptx"
    )
