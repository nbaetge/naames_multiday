...
We will filter out Eukaryotes, Archaea, chloroplasts and mitochondria, because we only intended to amplify bacterial sequences. It's good to check you don’t have anything lurking in the taxonomy table. 
...
pcoa.plot <- plot_ordination(rarefy_ps, pcoa,  title = "Bray Curtis PCoA", shape = "Depth.Interval", color = "Latitude") +
...
nmds.plot <- plot_ordination(rarefy_ps, nmds,  title = "NMDS", shape = "Depth.Interval", color = "Latitude") +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_point(alpha = 0.6, stroke = 2, size = 4) +
  geom_point(colour = "grey90", size = 1.5) +
  #geom_text(aes(label = Station), hjust = 0, vjust = 0, color = "black", alpha = 0.7, size = 3, position = position_jitter(), fontface = "bold") +
...
# CAP plot
cap.plot <- plot_ordination(ps_not_na, ordination = cap_ord,  shape = "Depth.Interval", color = "Latitude", axes = c(1,2)) + 
  geom_point(alpha = 0.6, stroke = 2, size = 4) +
  geom_point(colour = "grey90", size = 1.5) +
  scale_color_gradientn(colors = odv.colors) +
  custom_theme()  +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 10, fig.width = 22, fig.align = "center"}
ggplot(alphadiv, aes(x = `Depth Interval`, y = mean, group = `Depth Interval`)) +
  facet_grid(measure~ Season, scales = "free") +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 10, fig.width = 22, fig.align = "center"}
ggplot(alphadiv, aes(x = Season, y = mean, group = Season)) +
  facet_grid(`Depth Interval` ~ measure, scales = "free") +
  geom_boxplot(size = 0.5) +
  custom_theme()
```
