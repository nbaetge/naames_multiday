...
heat.order <- c("Acidimicrobiales", "Flavobacteriales", "OCS116_clade", "Rhodobacterales", "Rhodospirillales", "Rickettsiales", "SAR11_clade", "Oceanospirillales", "Salinisphaerales", "SAR406_clade*")
...
zheat.data <- zscore %>% 
 ...
 #  pivot_longer(., c(`Proteobacteria _ Alphaproteobacteria _ SAR11_clade _ SAR11_Ia`:`Chloroflexi _ SAR202_clade _ SAR202_Clade1 _ SAR202_Clade1`), names_to = "taxa", values_to = "relabund")
  ...
  pivot_longer(.,-c(Date,z), names_to = "taxa", values_to = "zscore") %>% 
  separate(taxa, into = c("p", "c", "o"), sep = " _ ") %>% 
  mutate(o = ifelse(o == "NA", "SAR406_clade*", o))


zheat <- zheat.data %>%
  ggplot(aes(x = Date, y = factor(o, levels = rev(heat.order)))) +
...
relaheat.data <- value.relabund %>% 
 ...
 #  pivot_longer(., c(`Proteobacteria _ Alphaproteobacteria _ SAR11_clade _ SAR11_Ia`:`Chloroflexi _ SAR202_clade _ SAR202_Clade1 _ SAR202_Clade1`), names_to = "taxa", values_to = "relabund")
  ...
  pivot_longer(.,-c(Date,z), names_to = "taxa", values_to = "relabund") %>% 
  separate(taxa, into = c("p", "c", "o"), sep = " _ ") %>% 
  mutate(o = ifelse(o == "NA", "SAR406_clade*", o))
...
relaheat <- relaheat.data %>%
  ...
  geom_tile(aes(fill = relabund), color = "white") +
   scale_fill_gradient2(low = "#0db5e6", mid = "white", high = "#d31f2a", midpoint = 0.25 ) +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 16, fig.width = 15, fig.align = "center"}


patchwork <- relaheat + zheat
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
sar11 <- relabund %>% 
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
sar324 <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Proteobacteria _ Deltaproteobacteria _ SAR324_clade(Marine_group_B)` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste(""))), y = expression(italic(paste("% Relative Abundance"))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
  coord_flip() +
  scale_y_continuous(expand = c(0,0)) +
  scale_shape_manual(values = c("Core" = 21, "Outside" = 24)) +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
sar202 <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Chloroflexi _ SAR202_clade _ SAR202_Clade3` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste(""))), y = expression(italic(paste("% Relative Abundance"))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
 ...
 guides(fill = F, color = F, shape = F, linetype = F) +
  ...
  theme(panel.spacing.x = unit(1, "cm"),
        axis.text.x = element_text(angle = 0),
        legend.key.size = unit(1, "cm")) +
  ggtitle("SAR202_clade2")

```

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
sar406 <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Marinimicrobia_(SAR406_clade) _ NA _ NA` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste(""))), y = expression(italic(paste("% Relative Abundance"))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
oceano <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Proteobacteria _ Gammaproteobacteria _ Oceanospirillales` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste(""))), y = expression(italic(paste(""))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
...
guides(fill = F, color = F, shape = F, linetype = F) +
  ...
  theme(panel.spacing.x = unit(1, "cm"),
        axis.text.x = element_text(angle = 0),
        legend.key.size = unit(1, "cm")) +
  ggtitle("Oceanospirillales")

```
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
flavo <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Bacteroidetes _ Flavobacteriia _ Flavobacteriales` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste(""))), y = expression(italic(paste(""))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
rhodo <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Proteobacteria _ Alphaproteobacteria _ Rhodobacterales` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste(""))), y = expression(italic(paste(""))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
salini <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Proteobacteria _ Gammaproteobacteria _ Salinisphaerales` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste("Depth, m"))), y = expression(italic(paste("% Relative Abundance"))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
  coord_flip() +
  scale_y_continuous(expand = c(0,0)) +
  scale_shape_manual(values = c("Core" = 21, "Outside" = 24)) +
...
guides(fill = F, color = F, shape = F, linetype = F) +
  ...
  theme(panel.spacing.x = unit(1, "cm"),
        axis.text.x = element_text(angle = 0),
        legend.key.size = unit(1, "cm")) +
  ggtitle("Salinisphaerales")

```

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
acidi <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Actinobacteria _ Acidimicrobiia _ Acidimicrobiales` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste("Depth, m"))), y = expression(italic(paste(""))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
  coord_flip() +
  scale_y_continuous(expand = c(0,0)) +
  scale_shape_manual(values = c("Core" = 21, "Outside" = 24)) +
...
guides(fill = F, color = F, shape = F, linetype = F) +
  ...
  theme(panel.spacing.x = unit(1, "cm"),
        axis.text.x = element_text(angle = 0),
        legend.key.size = unit(1, "cm")) +
  ggtitle("Acidimicrobiales")

```

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
rick <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Proteobacteria _ Alphaproteobacteria _ Rickettsiales` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste(""))), y = expression(italic(paste(""))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
...
guides(fill = F, color = F, shape = F, linetype = F) +
  ...
  theme(panel.spacing.x = unit(1, "cm"),
        axis.text.x = element_text(angle = 0),
        legend.key.size = unit(1, "cm")) +
  ggtitle("Rickettsiales")

```

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
rhodosp <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Proteobacteria _ Alphaproteobacteria _ Rhodospirillales` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste("Depth, m"))), y = expression(italic(paste(""))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
...
guides(fill = F, color = F, shape = F, linetype = F) +
  ...
  theme(panel.spacing.x = unit(1, "cm"),
        axis.text.x = element_text(angle = 0),
        legend.key.size = unit(1, "cm")) +
  ggtitle("Rhodospirillales")

```

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 9, fig.width = 8, fig.align = "center"}
ocs <- relabund %>% 
  ggplot(aes(x = z, y = relabund$`Proteobacteria _ Alphaproteobacteria _ OCS116_clade` * 100, group = interaction(Date, CampCN))) +
  geom_line(aes(linetype = eddy, color = Date), size = 0.7) +
  geom_point(aes(shape = eddy, fill = Date), size = 6, color = "black", stroke = 1, alpha = 0.7) + 
  labs(x = expression(italic(paste(""))), y = expression(italic(paste(""))), colour = "", fill = "", linetype = "Eddy Location", shape = "Eddy Location") +
  scale_x_reverse(breaks = pretty_breaks(), expand = c(0,0)) +
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 14, fig.width = 20, fig.align = "center"}

patchwork <- acidi + flavo + ocs + rhodo + rhodosp + rick + sar11 + oceano + salini +  sar406 +  sar324 + sar202
...
stack.data <- value.relabund %>% 
  select(c(1:12)) %>% 
  pivot_longer(-c(Date, z), names_to = "taxa", values_to = "relabund") %>% 
  group_by(Date, z) %>% 
  mutate(sum = sum(relabund))

stack.data %>% 
  ggplot(., aes(x =  Date, y = relabund)) + 
  facet_grid(rows = "z") +
  geom_bar(aes(fill = taxa), color = "black", stat = "identity", position = "stack", width = 0.25, stroke = 1) +
 scale_fill_manual(values = cal_palette("figmtn", n = 19, type = "continuous")) +
  ...
  ylab("Relative Abundance (Top 10 Taxa rep > 88% total community)") 
```
