...
mean(sample_sums(sub_ps)) #109,615
...
# Ordinate
nmds_all <- ordinate(sub_ps, method = "NMDS",  distance = "bray") # stress = 0.07, no convergence i.e. two axes not sufficient to view data
nmds_min <- ordinate(ps_min, method = "NMDS",  distance = "bray") # stress = 0.07, no convergence i.e. two axes not sufficient to view data
nmds_10k <- ordinate(ps_10k, method = "NMDS",  distance = "bray") # stress = 0.08, two solutions reached 
...
NMDS plots attempt to show ordinal distances between samples as accurately as possible in two dimensions. It is important to report the stress of these plots, because a high stress value means that the algorithm had a hard time representing the distances between samples in 2 dimensions (also if there is no convergence). The stress of all the plots was good (generally anything below 0.2 is considered acceptable). The subsampled to 10k reads was the only dataset where a convergent solution was reached. 
